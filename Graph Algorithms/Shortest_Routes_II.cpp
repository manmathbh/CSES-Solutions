...
void solve() {
    ll n, m, q;
    cin >> n >> m >> q;     //cities, roads, queries
    
    vector<vll> dist(n+2, vll(n+2, LONG_MAX));
    ...
    f(i, 1, n){
        f(j, 1, n){
            f(k, 1, n){
                if(dist[j][i] != LONG_MAX && dist[i][k] != LONG_MAX)    dist[j][k] = min(dist[j][k], dist[j][i]+dist[i][k]);
            }
        }
    }
    ...
    while(q--) {
        ...
        if(dist[u][v] == LONG_MAX)      cout << -1 << nl;
