...
void dfs(int r, int c, vector<string> &room, int n, int m) {
    if (r < 0 || r >= n || c < 0 || c >= m || room[r][c] == '#') return;
...
int countRooms(vector<string>& map, int n, int m) {
    int cnt = 0;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (map[i][j] == '.') {
